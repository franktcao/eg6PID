...
class EG5_test {
   ...
   TTree         *fChain;   //!pointer to the analyzed TTree or TChain
	...
	vector< vector< vector<TH2D*> > > 	e_2Cuts;
	vector< vector< vector<TH2D*> > >	p_2Cuts;
	vector< vector< vector<TH2D*> > >	gEC_2Cuts;
	vector< vector< vector<TH2D*> > >	gIC_2Cuts;
	vector< vector< vector<TH2D*> > >	he4_2Cuts;
	vector< vector< vector<TH2D*> > >	pi0sECEC_2Cuts;
	vector< vector< vector<TH2D*> > >	pi0sECIC_2Cuts;
	vector< vector< vector<TH2D*> > >	pi0sICIC_2Cuts;
	
	vector< vector< vector<TH1D*> > >	e_1Cuts;
	vector< vector< vector<TH1D*> > >	p_1Cuts;
	vector< vector< vector<TH1D*> > >	gEC_1Cuts;
	vector< vector< vector<TH1D*> > >	gIC_1Cuts;
	vector< vector< vector<TH1D*> > >	he4_1Cuts;
	vector< vector< vector<TH1D*> > >	pi0sECEC_1Cuts;
	vector< vector< vector<TH1D*> > >	pi0sECIC_1Cuts;
	vector< vector< vector<TH1D*> > >	pi0sICIC_1Cuts;
	...
	Int_t					pi0sECEC_ntests;
	vector<Float_t>	pi0sECEC4ReportCard;	// (6,-999.);
	Int_t					pi0sECIC_ntests;
	vector<Float_t>	pi0sECIC4ReportCard;	// (6,-999.);
	Int_t					pi0sICIC_ntests;
	vector<Float_t>	pi0sICIC4ReportCard;	// (6,-999.);
   ...
   virtual void     Show(Long64_t entry = -1);
	...
	virtual void	  fillHists(Int_t ipid, Int_t iIO, Int_t itest);
	...
	void initHists(){
		...
		vector<vector<TH2D*>> nullplot2Vec;
		vector<vector<TH1D*>> nullplot1Vec;
		// 	Electron Histograms	///////////////////////////////////////////////////////////////////////////////////////////////////////	
		////// 1D Histograms //////////////////////////////
		e_ntests = 6;
		e_n2plots = 7;
		...
		for( Int_t ii = 0 ; ii < e_n2plots; ii++){
			e_2Cuts.push_back(plot2Vec);
		}
		for( Int_t ii = 0 ; ii < e_n1plots; ii++){
			e_1Cuts.push_back(plot1Vec);
		}
		for( Int_t iIO = 0; iIO < 4; iIO++){
			...
			{	
				ntests = 1;	
				if(iIO == 0){
					onOffStat = "(All Negative Tracks)";
				}
				if(iIO == 3){
					onOffStat = "(All Electron Tests Passed)";
				}
			}
			for( Int_t itest = 0; itest < ntests ; itest++){
				if( iIO == 1)
				{ onOffStat = "(Passed only: Test " + to_string(itest) + ")";	}
				if( iIO == 2)
				{ onOffStat = "(Failed only: Test " + to_string(itest) + ")";	}
		...
		////// 2D Histograms //////////////////////////////
				string name = "eID_DCyVxShadow_" + to_string(iIO) + "_" +  to_string(testNum);
				string title = "pID: DC: Y vs X ( IC on DC Shadow )"+onOffStat;
				TH2D *curHist2 = new TH2D( name.c_str(), title.c_str(), 200, -60., 60., 200, -60, 60);
				curHist2->SetXTitle("X [cm] ");
				curHist2->SetYTitle("Y [cm] ");
				e_2Cuts[0][iIO][itest] = curHist2;
				
				name = "eID_DCyVx_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "eID: DC: y vs x (Zoomed Out)" + onOffStat;
				curHist2 = new TH2D( name.c_str(), title.c_str(), 200, -80., 80., 200, -80., 80.);
				curHist2->SetXTitle("X [cm] ");
				curHist2->SetYTitle("Y [cm] ");
				e_2Cuts[1][iIO][itest] = curHist2;

				name = "eID_CCphiVtheta_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "eID: CC: Phi vs Theta " + onOffStat;
				curHist2 = new TH2D( name.c_str(), title.c_str(), 200, 0., 50., 200, -40., 40.);
				curHist2->SetXTitle("Theta [deg] ");
				curHist2->SetYTitle("Phi [deg] ");
				e_2Cuts[2][iIO][itest] = curHist2;

				name = "eID_ECphiVtheta_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "eID: EC: Y vs X " + onOffStat;
				curHist2 = new TH2D( name.c_str(), title.c_str(), 200, -400., 400., 200, -400., 400.);
				curHist2->SetXTitle("X [cm] ");
				curHist2->SetYTitle("Y [cm] ");
				e_2Cuts[3][iIO][itest] = curHist2;
				
				name = "eID_ECeoVei_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "eID: EC: Eo vs Ei " + onOffStat;
				curHist2 = new TH2D( name.c_str(), title.c_str(), 200, 0., 0.6, 200, 0., 0.5);
				curHist2->SetXTitle("Ei [GeV] ");
				curHist2->SetYTitle("Eo [GeV] ");
				e_2Cuts[4][iIO][itest] = curHist2;
				
				name = "eID_ECetotOOpVp_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "eID: EC: Etot/p vs p " + onOffStat; 
				curHist2 = new TH2D( name.c_str(), title.c_str(), 200, 0.7, 5., 200, 0., 1.);
				curHist2->SetXTitle("p [GeV/c] ");
				curHist2->SetYTitle("Etot/p [c] ");
				e_2Cuts[5][iIO][itest] = curHist2;
				
				name = "eID_pPhiVpTheta_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "eID: DC: Phi vs Theta " + onOffStat; 
				curHist2 = new TH2D( name.c_str(), title.c_str(), 200, 0., 70., 200, -180., 180.);
				curHist2->SetXTitle("Theta [deg] ");
				curHist2->SetYTitle("Phi [deg] ");
				e_2Cuts[6][iIO][itest] = curHist2;
				
		////// 1D Histograms //////////////////////////////
				name = "eID_Vz_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "eID: Z-Vertex " + onOffStat; 
				TH1D *curHist1 = new TH1D( name.c_str(), title.c_str(), 200, -100., -35.);
				curHist1->SetXTitle("z-Vertex [cm] ");
				e_1Cuts[0][iIO][itest] = curHist1;

				name = "eID_CCnphe_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "eID: CC: 10*Number of Photoelectrons " + onOffStat; 
				curHist1 = new TH1D( name.c_str(), title.c_str(), 250, 0., 250.);
				curHist1->SetXTitle("10 * Number of Photoelectrons ");
				e_1Cuts[1][iIO][itest] = curHist1;

				name = "eID_DCphi_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "eID: DC: Phi " + onOffStat; 
				curHist1 = new TH1D( name.c_str(), title.c_str(), 250, -180., 180.);
				curHist1->SetXTitle("Phi [deg] ");
				e_1Cuts[2][iIO][itest] = curHist1;
			}
		}
		...
		p_n2plots = 3;
		p_n1plots = 2;
		plot2Vec = nullplot2Vec;
		plot1Vec = nullplot1Vec;
		for( Int_t ii = 0 ; ii < 4; ii++ ){
		...
		for( Int_t ii = 0 ; ii < p_n2plots; ii++){
			p_2Cuts.push_back(plot2Vec);
		}
		for( Int_t ii = 0 ; ii < p_n1plots; ii++){
			p_1Cuts.push_back(plot1Vec);
		}
		...
		for( Int_t iIO = 0; iIO < 4; iIO++){
			...
			{	ntests = 1;	
				if(iIO == 0){
					onOffStat = "(All Positive Tracks)";
				}
				if(iIO == 3){
					onOffStat = "(All Protons Tests Passed)";
				}
			}
			for( Int_t itest = 0; itest < ntests ; itest++){
				if( iIO == 1)
		...
		////// 2D Histograms //////////////////////////////
				string name = "pID_DCyVx_" + to_string(iIO) + "_" +  to_string(testNum);
				string title = "pID: DC Y vs X " + onOffStat; 
				TH2D *curHist2 = new TH2D( name.c_str(), title.c_str(), 200, -80., 80., 200, -80, 80);
				curHist2->SetXTitle("X [cm] ");
				curHist2->SetYTitle("Y [cm] ");
				p_2Cuts[0][iIO][itest] = curHist2;

				name = "pID_SCdbVp_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "pID: SC delta Beta vs p " + onOffStat; 
				curHist2 = new TH2D( name.c_str(), title.c_str(), 200, 0., 3., 200, -0.7, 0.7);
				curHist2->SetXTitle("p [GeV/c] ");
				curHist2->SetYTitle("dB ");
				p_2Cuts[1][iIO][itest] = curHist2;
				
				name = "pID_DCphiVtheta_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "pID: DC Phi vs Theta " + onOffStat; 
				curHist2 = new TH2D( name.c_str(), title.c_str(), 200, 0., 70., 200, -180., 180.);
				curHist2->SetXTitle("Theta [deg] ");
				curHist2->SetYTitle("Phi [deg] ");
				p_2Cuts[2][iIO][itest] = curHist2;

		////// 1D Histograms //////////////////////////////
				name = "pID_SCdb_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "pID: SC Delta Beta " + onOffStat; 
				TH1D *curHist1 = new TH1D( name.c_str(), title.c_str(), 200, -0.0525, 0.1);
				curHist1->SetXTitle("Theta [deg] ");
				p_1Cuts[0][iIO][itest] = curHist1;

				name = "pID_dVz_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "pID: Delta Vz " + onOffStat; 
				curHist1 = new TH1D( name.c_str(), title.c_str(), 200, -3., 6.9);
				curHist1->SetXTitle("z Vertex [cm] ");
				p_1Cuts[1][iIO][itest] = curHist1;
			}
		}
		/*
		// Photon in EC Histograms	///////////////////////////////////////////////////////////////////////////////////////////////////////	
		gEC_ntests = 3;
		gEC_n2plots = 2;
		gEC_n1plots = 1;
		plot2Vec = nullplot2Vec;
		...
		for( Int_t ii = 0 ; ii < gEC_n2plots; ii++){
			gEC_2Cuts.push_back(plot2Vec);
		}
		for( Int_t ii = 0 ; ii < gEC_n1plots; ii++){
			gEC_1Cuts.push_back(plot1Vec);
		}
		...
		////// 2D Histograms //////////////////////////////
				string name = "gECID_ECyVx_" + to_string(iIO) + "_" +  to_string(testNum);
				string title = "gECID: EC Y vs X " + onOffStat; 
				TH2D *curHist2 = new TH2D( name.c_str(), title.c_str(), 200, -400., 400., 200, -400, 400);
				curHist2->SetXTitle("X [cm] ");
				curHist2->SetYTitle("Y [cm] ");
				gEC_2Cuts[0][iIO][itest] = curHist2;

				name = "gECID_ECphiVtheta_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "gECID: EC: Phi vs Theta " + onOffStat; 
				curHist2 = new TH2D( name.c_str(), title.c_str(), 200, 0., 70., 200, -180., 180.);
				curHist2->SetXTitle("Theta [deg] ");
				curHist2->SetYTitle("Phi [deg] ");
				gEC_2Cuts[1][iIO][itest] = curHist2;
				

		////// 1D Histograms //////////////////////////////
				name = "gECID_SCdb_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "gECID: SC Delta Beta " + onOffStat; 
				TH1D *curHist1 = new TH1D( name.c_str(), title.c_str(), 200, 0.6, 1.4);
				curHist1->SetXTitle("Beta ");
				gEC_1Cuts[0][iIO][itest] = curHist1;
			}
		}
		*/
	/*	
		// Photon in IC Histograms	///////////////////////////////////////////////////////////////////////////////////////////////////////	
		gIC_ntests = 3;
		gIC_n2plots = 3;
		gIC_n1plots = 0;
		plot2Vec = nullplot2Vec;
		...
		for( Int_t ii = 0 ; ii < gIC_n2plots; ii++){
			gIC_2Cuts.push_back(plot2Vec);
		}
		for( Int_t ii = 0 ; ii < gIC_n1plots; ii++){
			gIC_1Cuts.push_back(plot1Vec);
		}
		...
		////// 2D Histograms //////////////////////////////
				string name = "gICID_ICyVx_" + to_string(iIO) + "_" +  to_string(testNum);
				string title = "gICID: IC Y vs X " + onOffStat; 
				TH2D *curHist2 = new TH2D( name.c_str(), title.c_str(), 200, -20., 20., 200, -20, 20);
				curHist2->SetXTitle("X [cm] ");
				curHist2->SetYTitle("Y [cm] ");
				gIC_2Cuts[0][iIO][itest] = curHist2;

				name = "gICID_ICthetaVnrg_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "gICID: IC: Theta vs Energy " + onOffStat; 
				curHist2 = new TH2D( name.c_str(), title.c_str(), 200, 0., 6., 200, 2.5, 14.5);
				curHist2->SetXTitle("Energy [GeV] ");
				curHist2->SetYTitle("Phi [deg] ");
				gIC_2Cuts[1][iIO][itest] = curHist2;
				
				name = "gICID_ICphiVtheta_" + to_string(iIO) + "_" +  to_string(testNum);
				title = "gICID: IC: Phi vs Theta " + onOffStat; 
				curHist2 = new TH2D( name.c_str(), title.c_str(), 200, 0., 70., 200, -180., 180.);
				curHist2->SetXTitle("Theta [deg] ");
				curHist2->SetYTitle("Phi [deg] ");
				gIC_2Cuts[2][iIO][itest] = curHist2;
		...
		for( Int_t ii = 0 ; ii < he4_n2plots; ii++){
			he4_2Cuts.push_back(plot2Vec);
		}
		for( Int_t ii = 0 ; ii < he4_n1plots; ii++){
			he4_1Cuts.push_back(plot1Vec);
		}
		...
		////// 2D Histograms //////////////////////////////
				string name = "he4ID_RTPCphiVtheta_" + to_string(iIO) + "_" +  to_string(testNum);
				string title = "he4ID: RTPC: Phi vs Theta " + onOffStat; 
				TH2D *curHist2 = new TH2D( name.c_str(), title.c_str(), 200, 10., 170., 200, 0, 360);
				curHist2->SetXTitle("Theta [deg] ");
				curHist2->SetYTitle("Phi [deg] ");
				he4_2Cuts[0][iIO][itest] = curHist2;

		////// 1D Histograms //////////////////////////////
				name = "he4ID_RTPC_L_Vz" + to_string(iIO) + "_" +  to_string(testNum);
				title = "he4ID: RTPC (Left): Z-Vertex " + onOffStat; 
				TH1D *curHist1 = new TH1D( name.c_str(), title.c_str(), 200, 10., 170.);
				curHist1->SetXTitle("z-vertex [mm] ");
				he4_1Cuts[0][iIO][itest] = curHist1;

				name = "he4ID_RTPC_R_Vz" + to_string(iIO) + "_" +  to_string(testNum);
				title = "he4ID: RTPC (Right): Z-Vertex " + onOffStat; 
				curHist1 = new TH1D( name.c_str(), title.c_str(), 200, 10., 170.);
				curHist1->SetXTitle("z-vertex [mm] ");
				he4_1Cuts[1][iIO][itest] = curHist1;
				
				name = "he4ID_RTPC_L_X2" + to_string(iIO) + "_" +  to_string(testNum);
				title = "he4ID: RTPC (Left): Chi^2 " + onOffStat; 
				curHist1 = new TH1D( name.c_str(), title.c_str(), 200, 0., 10.);
				curHist1->SetXTitle("X^2 ");
				he4_1Cuts[2][iIO][itest] = curHist1;
				
				name = "he4ID_RTPC_R_X2" + to_string(iIO) + "_" +  to_string(testNum);
				title = "he4ID: RTPC (Right): Chi^2 " + onOffStat; 
				curHist1 = new TH1D( name.c_str(), title.c_str(), 200, 0., 10.);
				curHist1->SetXTitle("X^2 ");
				he4_1Cuts[3][iIO][itest] = curHist1;
				
				name = "he4ID_RTPC_L_sdist" + to_string(iIO) + "_" +  to_string(testNum);
				title = "he4ID: RTPC (Left): sdist " + onOffStat; 
				curHist1 = new TH1D( name.c_str(), title.c_str(), 200, -6., 6.);
				curHist1->SetXTitle("sdist [mm] ");
				he4_1Cuts[4][iIO][itest] = curHist1;
				
				name = "he4ID_RTPC_R_sdist" + to_string(iIO) + "_" +  to_string(testNum);
				title = "he4ID: RTPC (Right): sdist " + onOffStat; 
				curHist1 = new TH1D( name.c_str(), title.c_str(), 200, -6., 6.);
				curHist1->SetXTitle("sdist [mm] ");
				he4_1Cuts[5][iIO][itest] = curHist1;
				
				name = "he4ID_RTPC_L_edist" + to_string(iIO) + "_" +  to_string(testNum);
				title = "he4ID: RTPC (Left): edist " + onOffStat; 
				curHist1 = new TH1D( name.c_str(), title.c_str(), 200, -5., 15.);
				curHist1->SetXTitle("edist [mm] ");
				he4_1Cuts[6][iIO][itest] = curHist1;
				
				name = "he4ID_RTPC_R_edist" + to_string(iIO) + "_" +  to_string(testNum);
				title = "he4ID: RTPC (Right): edist " + onOffStat; 
				curHist1 = new TH1D( name.c_str(), title.c_str(), 200, -5., 15.);
				curHist1->SetXTitle("edist [mm] ");
				he4_1Cuts[7][iIO][itest] = curHist1;
				
				name = "he4ID_RTPC_L_dZ" + to_string(iIO) + "_" +  to_string(testNum);
				title = "he4ID: RTPC (Left): delta Z " + onOffStat; 
				curHist1 = new TH1D( name.c_str(), title.c_str(), 200, -50., 50.);
				curHist1->SetXTitle("dZ [mm] ");
				he4_1Cuts[8][iIO][itest] = curHist1;
				
				name = "he4ID_RTPC_R_dZ" + to_string(iIO) + "_" +  to_string(testNum);
				title = "he4ID: RTPC (Right): delta Z " + onOffStat; 
				curHist1 = new TH1D( name.c_str(), title.c_str(), 200, -50., 50.);
				curHist1->SetXTitle("dZ [mm] ");
				he4_1Cuts[9][iIO][itest] = curHist1;
			}
		}
		// pi0sECEC Histograms	///////////////////////////////////////////////////////////////////////////////////////////////////////	
		pi0sECEC_ntests = 1;
		...
		for( Int_t ii = 0 ; ii < pi0sECEC_n2plots; ii++){
			pi0sECEC_2Cuts.push_back(plot2Vec);
		}
		for( Int_t ii = 0 ; ii < pi0sECEC_n1plots; ii++){
			pi0sECEC_1Cuts.push_back(plot1Vec);
		}
		...
		////// 2D Histograms //////////////////////////////
				string name = "pi0sECEC_phiVtheta_" + to_string(iIO) + "_" +  to_string(testNum);
				string title = "pi0sECEC: RTPC: Phi vs Theta " + onOffStat; 
				TH2D *curHist2 = new TH2D( name.c_str(), title.c_str(), 200, 0., 70., 200, -180, 180);
				curHist2->SetXTitle("Theta [deg] ");
				curHist2->SetYTitle("Phi [deg] ");
				pi0sECEC_2Cuts[0][iIO][itest] = curHist2;

		////// 1D Histograms //////////////////////////////
				name = "pi0sECEC__Vz" + to_string(iIO) + "_" +  to_string(testNum);
				title = "pi0sECEC: RTPC (Left): Z-Vertex " + onOffStat; 
				TH1D *curHist1 = new TH1D( name.c_str(), title.c_str(), 200, 0., 0.5);
				curHist2->SetXTitle("z-vertex [mm] ");
				pi0sECEC_1Cuts[0][iIO][itest] = curHist1;
			}
		}
		// pi0sECIC Histograms	///////////////////////////////////////////////////////////////////////////////////////////////////////	
		pi0sECIC_ntests = 1;
		...
		for( Int_t ii = 0 ; ii < pi0sECIC_n2plots; ii++){
			pi0sECIC_2Cuts.push_back(plot2Vec);
		}
		for( Int_t ii = 0 ; ii < pi0sECIC_n1plots; ii++){
			pi0sECIC_1Cuts.push_back(plot1Vec);
		}
		...
		////// 2D Histograms //////////////////////////////
				string name = "pi0sECIC_phiVtheta_" + to_string(iIO) + "_" +  to_string(testNum);
				string title = "pi0sECIC: RTPC: Phi vs Theta " + onOffStat; 
				TH2D *curHist2 = new TH2D( name.c_str(), title.c_str(), 200, 0., 70., 200, -180, 180);
				curHist2->SetXTitle("Theta [deg] ");
				curHist2->SetYTitle("Phi [deg] ");
				pi0sECIC_2Cuts[0][iIO][itest] = curHist2;

		////// 1D Histograms //////////////////////////////
				name = "pi0sECIC__Vz" + to_string(iIO) + "_" +  to_string(testNum);
				title = "pi0sECIC: RTPC (Left): Z-Vertex " + onOffStat; 
				TH1D *curHist1 = new TH1D( name.c_str(), title.c_str(), 200, 0., 0.5);
				curHist2->SetXTitle("z-vertex [mm] ");
				pi0sECIC_1Cuts[0][iIO][itest] = curHist1;
			}
		}
		// pi0sICIC Histograms	///////////////////////////////////////////////////////////////////////////////////////////////////////	
		pi0sICIC_ntests = 1;
		...
		for( Int_t ii = 0 ; ii < pi0sICIC_n2plots; ii++){
			pi0sICIC_2Cuts.push_back(plot2Vec);
		}
		for( Int_t ii = 0 ; ii < pi0sICIC_n1plots; ii++){
			pi0sICIC_1Cuts.push_back(plot1Vec);
		}
		...
		////// 2D Histograms //////////////////////////////
				string name = "pi0sICIC_phiVtheta_" + to_string(iIO) + "_" +  to_string(testNum);
				string title = "pi0sICIC: RTPC: Phi vs Theta " + onOffStat; 
				TH2D *curHist2 = new TH2D( name.c_str(), title.c_str(), 200, 0., 70., 200, -180, 180);
				curHist2->SetXTitle("Theta [deg] ");
				curHist2->SetYTitle("Phi [deg] ");
				pi0sICIC_2Cuts[0][iIO][itest] = curHist2;

		////// 1D Histograms //////////////////////////////
				name = "pi0sICIC__Vz" + to_string(iIO) + "_" +  to_string(testNum);
				title = "pi0sICIC: RTPC (Left): Z-Vertex " + onOffStat; 
				TH1D *curHist1 = new TH1D( name.c_str(), title.c_str(), 200, 0., 0.5);
				curHist2->SetXTitle("z-vertex [mm] ");
				pi0sICIC_1Cuts[0][iIO][itest] = curHist1;
			}
		}
		*/
	}
	...
	Double_t getCorrectedVzShift(Int_t ipart){
		...
		// 	Only needed for electron since its beam is what is changing
	...
	Double_t getDeltaT(Int_t ipart){
		...
		Double_t measTime = sc_t[sc[ipart]-1];
	...
	Bool_t isPhotonEC(Int_t ipart){
		...
		vector<Float_t> gECAllFail( gEC_ntests, -999. );
		gECReportCard = gECAllFail;
		
		if( 	q[ipart] == 0					){  			// Charge Cut
			gECAllFail.assign( gEC_ntests, 0 ); 
			gECReportCard = gECAllFail;	
			if(	isG_ECEnergyCut(ipart)		){	gECReportCard[0] = 1;	}				// EC Energy Cut
			if(	isG_ECFiducialCut(ipart)	){	gECReportCard[1] = 1;	}				// EC Fiducial Cut
			//		if(	isG_vertexCut(ipart)			)				// NOT USED	
			if(	isG_betaCut(ipart)			){	gECReportCard[2] = 1;	}
		}
		else{	return false;	}
		
		if( gECReportCard == gECAllFail )
		{	return false;	}
		return true;
	}
	...
	Bool_t isPhotonIC(Int_t ipartIC){					// There are many photons produced in the IC for a given event. Thus we need a different PID
		...
		vector<Float_t> gICAllFail( gIC_ntests, 0. );
		gICReportCard = gICAllFail;
		
		if(	isG_ICGoodChannel(ipartIC)			){	gICReportCard[0] = 1;	}
		if(	isG_ICFiducialCut(ipartIC)			){ gICReportCard[1] = 1;	}
		if(	isG_ICThetaEnergyCut(ipartIC)		){	gICReportCard[2] = 1;	}
		
		if( gICReportCard == gICAllFail )
		{	return false;	}
		return true;
	}
...
//		return true;	
	...
	Bool_t isG_ECFiducialCut(Int_t ipart){
		...
		TVector3 ecHitXYZ;
		ecHitXYZ.SetXYZ(ech_x[ec[ipart]-1],ech_y[ec[ipart]-1],ech_z[ec[ipart]-1]);			// EC Hit Position
		TVector3 ecHitUVW;
