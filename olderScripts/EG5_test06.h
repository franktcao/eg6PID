...
Bool_t isProton(Int_t ipart){
	if(	eIndex > 0						)				// There is an accompanying electron
	if( 	q[ipart] > 0					)	  			// Charge Cut
...
//	if(	eIndex > 0									)				// There is an accompanying electron
//	if( 	q_tot[ipartGC] > 0								)	  			// Charge Cut
//	if( 	dc_stat[dc[ipartGC]] > 0				)				// Checks to see if DC is good for this run
//	if( 	sc_stat[sc[ipartGC]] > 0				)				// Checks to see if SC is good for this run
	if( 	r_0[ipartGC] > 0							)	  			// 
...
//	if( 	phiRTPC < 90. || phiRTPC > 270.		)	  			// Right siee of RTPC (Also just {not Left}: pigeon hole, de morgan's, common sense )
	...
	if( 	Abs(sdist[ipartGC]) < 2.				)	  			// Distance from cathode to first ionization point threshold Cut
	if( 	-1.0 < eedist && eedist < 5.			)	  			// Distance from cathode to first ionization point threshold Cut
	if(	isHe4_trackHelix(ipartGC)				)
	if(	isHe4_sDist(ipartGC)						)
	if(	isHe4_eDist(ipartGC)						)
...
vector<vector<TLorentzVector>> getPi0s(vector<TLorentzVector> photonsEC, vector<TLorentzVector> photonsIC){
	vector<vector<TLorentzVector>> allPi0s;
	vector<TLorentzVector> nullVect;
	vector<TLorentzVector> pi0ECEC = getPi0sFromPhotons(photonsEC, nullVect);
	allPi0s.push_back(pi0ECEC);

	vector<TLorentzVector> pi0ECIC = getPi0sFromPhotons(photonsEC, photonsIC);
	allPi0s.push_back(pi0ECIC);

	vector<TLorentzVector> pi0ICIC = getPi0sFromPhotons(photonsIC, nullVect);
	allPi0s.push_back(pi0ICIC);
...
vector<TLorentzVector> getPi0sFromPhotons(vector<TLorentzVector> photons1, vector<TLorentzVector> photons2){
	...
	for( Int_t ii = 0; ii < (Int_t) photons1.size(); ii++){
		...
		{	 lowIndex = ii;		 }
		for( Int_t jj = 0; lowIndex < jj && jj < (Int_t) photons2.size(); jj++){
...
vector<vector<TLorentzVector>> getPi0sV2(vector <TLorentzVector> photonsEC, vector<TLorentzVector> photonsIC){
	vector<vector<TLorentzVector>> allPi0s;
	vector<TLorentzVector> pi0ECEC = getPi0sSameDetector(photonsEC);
	allPi0s.push_back(pi0ECEC);

	vector<TLorentzVector> pi0ECIC = getPi0sDiffDetector(photonsEC, photonsIC);
	allPi0s.push_back(pi0ECIC);

	vector<TLorentzVector> pi0ICIC = getPi0sSameDetector(photonsIC);
	allPi0s.push_back(pi0ICIC);
...
vector<TLorentzVector> getPi0sSameDetector(vector<TLorentzVector> photons){
	vector<TLorentzVector> pi0s;
	for( Int_t ii = 0; ii < (Int_t) photons.size(); ii++){
		for( Int_t jj = ii + 1; jj < (Int_t) photons.size(); jj++){
			TLorentzVector pi0Candidate = photons[ii] + photons[jj];
			Double_t	invMass = pi0Candidate.M();
			if( 0.01 < invMass && invMass > 1.00 )
			{	pi0s.push_back(pi0Candidate);	} 
		}
...
vector<TLorentzVector> getPi0sDiffDetector(vector<TLorentzVector> photons1, vector<TLorentzVector> photons2){
	vector<TLorentzVector> pi0s;
	for( Int_t ii = 0; ii < (Int_t) photons1.size(); ii++){
		for( Int_t jj = 0; jj < (Int_t) photons2.size(); jj++){
			TLorentzVector pi0Candidate = photons1[ii] + photons2[jj];
			Double_t	invMass = pi0Candidate.M();
			if( 0.01 < invMass && invMass > 1.00 )
			{	pi0s.push_back(pi0Candidate);	} 
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////   Photon ID Cut Functions >	
/* SKIP FOR NOW
void correctRTPCVariables()
{
	for(int ipartRTPC = 0; ipartRTPC < rtpc_npart; ipartRTPC++)
   {
		Int_t ipartCorr = rtpc_gcpb[ipartRTPC];		// Corrected index
		// Variables from gcpb bank where we need the index conversion :rtpc_gcpb[i]
		pid_tpc[ipartRTPC]			= pid[ipartCorr];
		x_tpc[ipartRTPC]				= x[ipartCorr];
		y_tpc[ipartRTPC]				= y[ipartCorr];
		z_tpc[ipartRTPC]				= z[ipartCorr];
		dedx_tpc[ipartRTPC]			= dedx[ipartCorr];
		px_tpc[ipartRTPC]				= px[ipartCorr];
		py_tpc[ipartRTPC]				= py[ipartCorr];
		pz_tpc[ipartRTPC]				= pz[ipartCorr];
		ptot_tpc[ipartRTPC]			= p_tot[ipartCorr];
		x2_tpc[ipartRTPC]				= x2[ipartCorr];
		theta_tpc[ipartRTPC]			= theta[ipartCorr];
		charge_tpc[ipartRTPC]		= charge[ipartCorr];
		dca_tpc[ipartRTPC]			= dca[ipartCorr];
		index_tpc[ipartRTPC]			= index[ipartCorr];
		phi_tpc[ipartRTPC]			= phi[ipartCorr];
		vtl_tpc[ipartRTPC]			= vtl[ipartCorr];
		sdist_tpc[ipartRTPC]			= sdist[ipartCorr];
		edist_tpc[ipartRTPC]			= edist[ipartCorr];
		npts_tpc[ipartRTPC]			= npts[ipartCorr];
		r0_tpc[ipartRTPC]				= r_0[ipartCorr];
		fiterr_tpc[ipartRTPC]		= fiterr[ipartCorr];
		tothits_tpc[ipartRTPC]		= tothits[ipartCorr];
		npd_track_tpc[ipartRTPC]	= npd_track[ipartCorr];
		npd_event_tpc[ipartRTPC]	= npd_event[ipartCorr];
		bonus_bits_tpc[ipartRTPC]	= bonus_bits[ipartCorr];
		qtot_tpc[ipartRTPC]			= q_tot[ipartCorr];
		x_start_tpc[ipartRTPC]		= x_start[ipartCorr];
		y_start_tpc[ipartRTPC]		= y_start[ipartCorr];
		z_start_tpc[ipartRTPC]		= z_start[ipartCorr];
		x_end_tpc[ipartRTPC]			= x_end[ipartCorr];
		y_end_tpc[ipartRTPC]			= y_end[ipartCorr];
		z_end_tpc[ipartRTPC]			= z_end[ipartCorr];

      // Varibles from the corrected rtpc bank
		rtpc_id1_c[ipartRTPC]		= rtpc_id1[ipartRTPC];
		rtpc_id2_c[ipartRTPC]		= rtpc_id2[ipartRTPC];
		rtpc_id3_c[ipartRTPC]		= rtpc_id3[ipartRTPC];
		rtpc_id4_c[ipartRTPC]		= rtpc_id4[ipartRTPC];
		rtpc_id5_c[ipartRTPC]		= rtpc_id5[ipartRTPC];
		rtpc_p1_c[ipartRTPC]			= rtpc_p1[ipartRTPC];
		rtpc_p2_c[ipartRTPC]			= rtpc_p2[ipartRTPC];
		rtpc_p3_c[ipartRTPC]			= rtpc_p3[ipartRTPC];
		rtpc_p4_c[ipartRTPC]			= rtpc_p4[ipartRTPC];
		rtpc_p5_c[ipartRTPC]			= rtpc_p5[ipartRTPC];
		rtpc_poverq_c[ipartRTPC]	= rtpc_poverq[ipartRTPC];
		rtpc_dedx_c[ipartRTPC]		= rtpc_dedx[ipartRTPC];
		rtpc_dedx2_c[ipartRTPC]		= rtpc_dedx2[ipartRTPC];
		rtpc_theta_c[ipartRTPC]		= rtpc_theta[ipartRTPC];
		rtpc_phi_c[ipartRTPC]		= rtpc_phi[ipartRTPC];
		rtpc_vz_c[ipartRTPC]			= rtpc_vz[ipartRTPC];
		rtpc_bad_c[ipartRTPC]		= rtpc_bad[ipartRTPC];
		rtpc_gcpb_c[ipartRTPC]		= rtpc_gcpb[ipartRTPC];
	}
}
*/
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
{
	N = (statc[ipartIC]-statc[ipartIC]%10000)/10000 -1; 		// N : Hit ID in ICHB   
	ic_x = ich_xgl[N];													// x coordinate in ICHB
	ic_y = ich_ygl[N];
...
Bool_t isG_ICThetaEnergyCut(Double_t X, Double_t Y)
{
	// 1  2
	//
	//    3
