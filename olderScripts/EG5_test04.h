...
class EG5_test {
   ...
   TH1D			  *h1phi;
	...
	TObjArray					hList2;
	TObjArray					hList1;
	//TVector3 		trackECIntPos0;
	//TVector3 		trackECIntDir0;
	Double_t			myTheta;
	Double_t			myPhi;
	Double_t			etote;
	Int_t				sector;
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   virtual void     Show(Long64_t entry = -1);
	...
	//virtual Bool_t   isEVertexCut(Int_t partIndex);
	//virtual Bool_t	  isElectron(Int_t partIndex);
...
{	// Reads data from table, puts them into vectors, checks to see if vectors are not nonsensical, binary searches for certain value, returns the required double 
   ...
   // Declare table columns:
	...
	if( strcmp(fileName0, "EG6ev2file.dat") )
	{
	}
	else if( strcmp(fileName0, "EG6ECsampcorr.dat") )
	{
	}
	else {cout << "NOT A GOOD FILE" <<endl; return outVec;}
	//fileName0.append(".txt");	
	const char* fileName;
...
// OPTIMIZING
/*
Bool_t isElectron(Int_t partIndex){
	if( 	!(stat[partIndex] > 0)					)		hCuts1[5].Fill(0);		// Checks to see if it is a good run
	if( 	!(dc_stat[dc[partIndex]] > 0)		)		hCuts1[5].Fill(1);		// Checks to see if DC is good for this run
	if( 	!(q[partIndex] < 0)						)	  	hCuts1[5].Fill(2);		// Charge Cut
	if( 	!(nphe[cc[partIndex]] > 2.0) 		) 		hCuts1[5].Fill(3);		// CC Cut
	if( 	!(p[partIndex] > 0.7) 					)		hCuts1[5].Fill(4);		// Momentum Cut for delta electrons
	if( 	!(p[partIndex] < 6.0) 					)		hCuts1[5].Fill(5);		// 	(in GeV/c)
	if( 	!isE_VertexCut(partIndex) 			)		hCuts1[5].Fill(6);		// Make sure particle is not electrons coming from window
	if( 	!isE_ECFiducialCut(partIndex) 	)		hCuts1[5].Fill(7);		// Make sure particle is in the EC fiducial region 
	if( 	!isE_ECEnergyCut(partIndex) 		)		hCuts1[5].Fill(8);		// Make sure enough energy is deposited in the EC
	if( 	!isE_CCFiducialCut(partIndex) 	)		hCuts1[5].Fill(9);		// Make sure particle is in the CC fiducial region
	if( 	!isE_DCFiducialCut(partIndex) 	)		hCuts1[5].Fill(10);		// Make sure particle is in DC fiducial region
	if(0){
		return true;
	}
	return false;
...
Bool_t isE_VertexCut(Int_t partIndex){
//	 return true;
...
Bool_t isE_ECFiducialCut(Int_t partIndex){
//	return true;
...
Bool_t isE_ECEnergyCut(Int_t partIndex){
//	return true;
	// EC Energy Cuts
	...
	if( etot[ec[partIndex]]	 > 0 )
	{
		...
		Double_t etote = (etot[ec[partIndex]] < ec_ei[ec[partIndex]] + ec_eo[ec[partIndex]]) ? ec_ei[ec[partIndex]] + ec_eo[ec[partIndex]] : etot[ec[partIndex]];
...
Bool_t isE_DCFiducialCut(Int_t partIndex){
//	return true;
	// DC Fiducial Cut
...
Bool_t isE_CCFiducialCut(Int_t partIndex){
//	return true;
	// CC Fiducial Cuts
