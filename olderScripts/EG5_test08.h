...
class EG5_test {
   ...
   TTree         *fChain;   //!pointer to the analyzed TTree or TChain
	...
	TVector3 dcHitPos, dcHitDir;
	Double_t dcX, dcY, dcTheta, dcPhi, dcICx, dcICy;
	TVector3 ccHitECPos;
	...
	vector< vector<TH2D*> >  				p_2CutsAllNone;
	...
	vector< vector< vector<TH2D*> > >	p_2CutsIO;
	...
	vector< vector<TH1D*> >  				p_1CutsAllNone;
	...
	vector< vector< vector<TH1D*> > >	p_1CutsIO;
   ...
   virtual void     Show(Long64_t entry = -1);
	...
	virtual void	  fillHists(Int_t ipid, Int_t iIoAn, Int_t iIO);
	...
	/*	
		...
		*/
		...
		e2Axes.push_back(e2xy);
		e2xy.clear();
		e2Binfo.push_back({ 200.,	0.	  ,	 5.  ,		200. ,    0.   ,	1.  });
		...
		///////// 6 vectors that hold one of the 6 categories ( DC Y vs X, CC phi vs theta, etc)
		//////////// 6 plots, one of each tells wheter it passes a given electron test for that given plot (isElectron tests)
		/////////////// 2 vectors that will tell whether it passes no test, the test, all other tests, all tests.... respectively
		vector<vector<TH2D*>> nullJj2;
		vector<vector<TH1D*>> nullJj1;
		
		vector<TH2D*> nullKk2;
		vector<TH1D*> nullKk1;
		
		vector<vector<TH2D*>> passesJj2;
		vector<vector<TH1D*>> passesJj1;
		
		vector<TH2D*> passesKk2;
		vector<TH1D*> passesKk1;
		...
		for(int ii = 0; ii < TMath::Max(e2length, e1length); ii++){	
			...
			passesKk1 = nullKk1;
				for(int kk = 0; kk < 2; kk++){
					if( ii < e2length){	
						name = e2Names[ii] + "_" + to_string(jj+1) + "_" + to_string(kk+1);
						title = e2Titles[ii] + " " + to_string(jj+1) + " " + to_string(kk+1);
						...
						passesKk2.push_back(curHist2);
					}
					if( ii < e1length){	
						name = e1Names[ii] + "_" + to_string(jj+1) + "_" + to_string(kk+1);
						title = e1Titles[ii] + " " + to_string(jj+1) + " " + to_string(kk+1);
						...
						passesKk1.push_back(curHist1);
					}
				}
				passesJj2.push_back(passesKk2);
				passesJj1.push_back(passesKk1);
			}
			e_2CutsIO.push_back(passesJj2);	
			e_1CutsIO.push_back(passesJj1);	
		}
		for(int ii = 0; ii < TMath::Max(e2length, e1length); ii++){	
			passesKk2 = nullKk2;
			passesKk1 = nullKk1;
			...
			for(int kk = 0; kk < 2; kk++){
				...
				if( ii < e2length){	
					name = e2Names[ii] + "_" + to_string(jj) + "_" + to_string(kk);
					title = e2Titles[ii] + " " + to_string(jj) + " " + to_string(kk);
					TH2D *curHist2 = new TH2D( name.c_str(), title.c_str(), (int) e2Binfo[ii][0], e2Binfo[ii][1], e2Binfo[ii][2], (int) e2Binfo[ii][3], e2Binfo[ii][4], e2Binfo[ii][5]);
					axisX = (string) e2Axes[ii][0];
					axisY = (string) e2Axes[ii][1];
					curHist2->SetXTitle(axisX.c_str());
				...
				if( ii < e1length){	
					name = e1Names[ii] + "_" + to_string(jj) + "_" + to_string(kk);
					title = e1Titles[ii] + " " + to_string(jj) + " " + to_string(kk);
					TH1D *curHist1 = new TH1D( name.c_str(), title.c_str(), (int) e1Binfo[ii][0], e1Binfo[ii][1], e1Binfo[ii][2]);
					axisX = (string) e1Axes[ii];
					curHist1->SetXTitle(axisX.c_str());
					passesKk1.push_back(curHist1);
			...
			e_2CutsAllNone.push_back(passesKk2);	
			e_1CutsAllNone.push_back(passesKk1);	
	}
	}
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	Bool_t isElectron(Int_t ipart){
		...
		if( 	nphe[cc[ipart]-1] > 2.0 				) 				// CC Cut
			{	
			eAllFail.assign( e_ntests, 0); // =  { 0, 0, 0, 0, 0, 0} ;
			eReportCard = eAllFail;	
...
*/
	...
	Bool_t isE_ECEnergyCut(Int_t ipart){
		...
		//	cout << "4.5" << endl;
			Double_t sampFrac = eTot/p[ipart];
		//	cout << "5" << endl;
...
//		if( S==3 || S==4 || S==5 ) 
//		if( X*Tan(goodRelAngL)< Y && Y <X*Tan(goodRelAngR) )
		if( S==3 || S==4 || S==5 || ( X*Tan(goodRelAngL)< Y && Y <X*Tan(goodRelAngR) ) )
		if( S==1 || S==2 || S==6 || ( X*Tan(goodRelAngR)< Y && Y <X*Tan(goodRelAngL) ) )
		{	return true;	}
//		if( S==1 || S==2 || S==6 )
//		if( X*Tan(goodRelAngR)< Y && Y <X*Tan(goodRelAngL) ) 
//		{	return true;	}
